using System;
using System.Collections.Generic;
using IdmNet.Models;
using Xunit;
using FluentAssertions;
// ReSharper disable ObjectCreationAsStatement
// ReSharper disable UseObjectOrCollectionInitializer

namespace IdmNet.Models.Tests
{
    public class FunctionTests
    {
        private Function _it;

        public FunctionTests()
        {
            _it = new Function();
        }

        [Fact]
        public void It_has_a_paremeterless_constructor()
        {
            _it.ObjectType.Should().Be("Function");
        }

        [Fact]
        public void It_has_a_constructor_that_takes_an_IdmResource()
        {
            var resource = new IdmResource
            {
                DisplayName = "My Display Name",
                Creator = new Person { DisplayName = "Creator Display Name", ObjectID = "Creator ObjectID"},
            };
            var it = new Function(resource);

            it.ObjectType.Should().Be("Function");
            it.DisplayName.Should().Be("My Display Name");
            it.Creator.DisplayName.Should().Be("Creator Display Name");
        }

        [Fact]
        public void It_has_a_constructor_that_takes_an_IdmResource_without_Creator()
        {
            var resource = new IdmResource
            {
                DisplayName = "My Display Name",
            };
            var it = new Function(resource);

            it.DisplayName.Should().Be("My Display Name");
            it.Creator.Should().Be(null);
        }

        [Fact]
        public void It_throws_when_you_try_to_set_ObjectType_to_anything_other_than_its_primary_ObjectType()
        {
            Action action = () => _it.ObjectType = "Invalid Object Type";
            action.ShouldThrow<InvalidOperationException>();
        }

        [Fact]
        public void It_can_get_and_set_Assembly()
        {
            // Act
            _it.Assembly = "A string";

            // Assert
            _it.Assembly.Should().Be("A string");
        }


        [Fact]
        public void It_can_get_and_set_FunctionName()
        {
            // Act
            _it.FunctionName = "A string";

            // Assert
            _it.FunctionName.Should().Be("A string");
        }


        [Fact]
        public void It_can_get_and_set_Namespace()
        {
            // Act
            _it.Namespace = "A string";

            // Assert
            _it.Namespace.Should().Be("A string");
        }


        [Fact]
        public void It_can_get_and_set_FunctionParameters()
        {
            // Act
            _it.FunctionParameters = "A string";

            // Assert
            _it.FunctionParameters.Should().Be("A string");
        }


        [Fact]
        public void It_can_get_and_set_ReturnType()
        {
            // Act
            _it.ReturnType = "A string";

            // Assert
            _it.ReturnType.Should().Be("A string");
        }


    }
}
